...
CONTENT_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    ("RESTRICT-GAMBLING", re.compile(r"토토|바카라|슬롯머신|배팅\s*사이트|카지노")),
    ("PROHIB-COUNTERFEIT", re.compile(r"정품\s*급|이미테이션|짝퉁|A급\s*미러")),
]
...
def check_absence(snap: PageSnapshot, ad_copy: str) -> list[Finding]:
    ...
    if not _search_any(text, CONTACT_PATTERNS):
        findings.append(_make("MIS-BUSINESS-IDENTITY"))
...
def _keywords(text: str) -> set[str]:
    """한글·영문 토큰. 조사·기호를 대충 털어낸다."""
    tokens = re.findall(r"[가-힣]{2,}|[A-Za-z]{3,}", text)
    return {t.lower() for t in tokens if len(t) >= KEYWORD_MIN_LEN}
